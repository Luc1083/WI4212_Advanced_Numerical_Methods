...
def run_advec(T,CFL,grid_type,Nx):

    non_uniform = grid_type
...
def main():
    ...
    configs = list(it.product(CFL_list,T_list,grid_list,Nx_list))
    
    for CFL, T, grid, Nx in configs:
        print(f"Now Running advec for CFL = {CFL}, T = {T}, non-uniform = {grid}, Nx = {Nx}")
        
        run_advec(T,CFL,grid,Nx)
        print("Finished!")
        print()
